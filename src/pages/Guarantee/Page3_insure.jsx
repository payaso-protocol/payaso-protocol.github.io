...
import { onExercise, bids } from "../../abi/order";
  ...
  getAllData = async () => {
    ...
    const super_data = new_data_2.map((item) => {
      ...
      return {
        ...
        _expiry: web3.utils.fromWei(item.sellInfo.longInfo._expiry),
    ...
    const super_data_3 = super_data_2.sort((a1, a2) => {
      return a2.sort - a1.sort;
    });

    return this.getRemain(super_data_3);
  };

  getRemain = (super_data) => {
    this.setState({
      ListArr: super_data,
      backup_list: super_data,
    });
    // let status = ''
    // Promise.all(
    //   super_data.map(async (item, index) => {
    //     console.log(item)
    //     const res = await bids(item.bidID, "defaule", item._collateral);
    //     console.log(res)
    //     return (item = {
    //       ...item,
    //       // remain: res,
      ...
      case "activate":
        return (
          <Warning
            title={t("t101")}
            content={t("t102", {
              price1: this.state.currentListItem.vol,
              symbol1: this.state.currentListItem._collateral,
              price2:
                this.state.currentListItem.vol *
                ...
                {ListArr.map((item, index) => {
                  return (
                    <tr key={item + index}>
                      ...
                      <td>{item.vol * item._strikePrice}</td>
                      <td>
                        {parseFloat(item.price) / parseFloat(item._strikePrice)}
                        {item.settleToken}
                      </td>
                      <td>
                        {parseFloat(item.price) * parseFloat(item.vol)}
                        {item.settleToken}
                      </td>
                      <td>
            ...
            return (
              <div className="option_item" key={index}>
                ...
                <div className="option_item_text">
                  <p>
                    <span>{t("t39")}</span>
                    <span>{item.vol * item._strikePrice}</span>
                  </p>
                  <p>
                    <span>{t("t32")}</span>
                    <span>{item.vol * item._strikePrice}</span>
                ...
                <div className="option_item_text">
                  <p>
                    ...
                    <span>
                      {(
                        parseFloat(item.price) * parseFloat(item.volume)
                      ).toFixed(4)}
                      {item.settleToken}
                    </span>
                  </p>
