...
export const mutations = {
    ...
    SET_HELMET_VARIETIES(state, data) { // 将对应的key存进去
        state.helmetVarieties = data;
    },
    SET_TOTAL_HELMETS_BORROWED_VOLUME(state, data) { // 保险交易过的资金量  （保单数量累加， vol 用抵押物处理）
        state.totalHelmetsBorrowedVolume = data;
    },
    SET_LONG_TOKEN_CREATED_VOLUME(state, data) { // 24小时Long token 铸造量
...
export const actions = {
        ...
        getOptionCreatedLog((err, data) => {
            ...
            data.forEach((item) => {
                ...
                if (_col === 'USDT' || _col === 'USDC') {
                    ...
                    resultItem._strikePrice = String(toRounding(window.WEB3.utils.fromWei(String(resultItem._strikePrice), 'mether'), 0));
        ...
        getMint((err, data) => {
            ...
            for (let i = 0; i < data.length; i++) {
                if (last - data[i].blockNumber < h24) {
                    ...
                    longTokenCreatedVolume += Number(fromWei(data[i].returnValues.vol, _col));
                }
            }
    ...
    async mapAbountInfoSell({ commit, state }, data) {
        ...
        for (let key in sellMap) {
            ...
            if ( Number(item.price) < Math.pow(10, 30) && (
                (longInfo && charID === 1) ||
    ...
    async getHatList({ commit, state }, data) {
        ...
        for (let i = 0; i < list.length; i++) {
            item = list[i];
            // 标的物
            const _collateral = newGetSymbol(
                item.longInfo._collateral.toLowerCase()
            );
            // 抵押物
            const _underlying = newGetSymbol(
